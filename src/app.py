...
def build_graph():
    ...
    workflow.add_node("create_file_node", lambda s: {**s, "result": create_file(s["filename"])})
    workflow.add_node("write_code_node", lambda s: {**s, "result": write_code(s["filename"], s["details"])[0]})
    workflow.add_node("summarize_node", lambda s: {**s, "result": summarize_text(s["details"])})
    workflow.add_node("chat_node", lambda s: {**s, "result": general_chat(s["details"])})
    ...
    workflow.add_conditional_edges(
        "intent_node",
        tool_router,
        {k: k for k in ["create_file_node", "write_code_node", "summarize_node", "chat_node"]}
    )
