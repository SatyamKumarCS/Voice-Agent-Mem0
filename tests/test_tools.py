...
test("File has correct content", open("output/hello.txt").read() == "Hello World!")

result = create_file("../../etc/passwd", "hack")
test("Path traversal blocked", "etc" not in result or os.path.exists("output/passwd"))
...
r = execute_tool({"intent": "create_file", "details": "", "filename": "test.txt"})
